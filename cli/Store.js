...
class Store {
    constructor() {
        this.dataFile = `${x(
            'npm -g root'
        ).stdout.trim()}/volons/cli/volons-cli/${_DATASTORE_FILENAME_}`;
    ...
    _load() {
        ...
        try {
            ...
            p.error(
                `Error: Loadding DataStore File: ${this.dataFile}`,
                exception
            );
            return false;
        }
        ...
        try {
            ...
            p.info(fileStr);
            p.error(
                `Error: Cannot parse DataStore file: ${this.dataFile}`,
                exception
            );
            return false;
        }
    ...
    _save() {
        ...
        try {
            ...
            p.error(
                `Error: Cannot save DataStore to file: ${this.dataFile}`,
                exception
            );
            return false;
        }
    ...
    _get(key) {
        if (!this.store) {
            throw new Error(
                `Cannont get key: ${key}. DataStore: ${this.dataFile} is undefined.`
            );
        }
        ...
        throw new Error(
            `Key: ${key} is undefined in DataStore: ${this.dataFile}.`
        );
    }
    ...
    removeDockerCompose(dockerComposeId) {
        ...
        const selectedDockerCompose = this.store.dockerComposes.filter(
            dc => dc.id === dockerComposeId
        );
        ...
        if (selectedDockerCompose && selectedDockerCompose.length === 1) {
            try {
                ...
                p.warn(
                    `Cannot delete docker-compose.yml: ${selectedDockerCompose[0].filePath}. ${exception}`
                );
            }
            this.store.dockerComposes = this.store.dockerComposes.filter(
                dc => dc.id !== dockerComposeId
            );
            return this._save();
        }
    ...
    addDockerCompose(dockerCompose) {
        this.store.dockerComposes = this.store.dockerComposes.filter(
            dc => dc.filePath !== dockerCompose.filePath
        );
        dockerCompose.id = Math.random()
            .toString(36)
            .substr(2);
    ...
    getSubnetIpInc(filePath) {
        const currentDockerCompose = this.store.dockerComposes.filter(
            dc => dc.filePath === filePath
        );
