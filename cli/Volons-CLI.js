...
const helpFilePath =
    x('npm root -g', { options: 'quiet', sync: true }).stdout.trim() +
    '/volons/cli/help/';
...
const droneNameSequence = function(entryVal, cd, droneCounter) {
    if (
        entryVal === '' ||
        entryVal.toLowerCase() === 'yes' ||
        entryVal.toLowerCase() === 'y'
    ) {
        p.logNoCR(
...
const homePositionSequence = function(entryVal) {
    ...
    if (
        ...
        p.error(
            `Error: '${entryVal}': This location does't match a regular position : '[Latitude], [Longitude], [Altitude (meter)] string.`
        );
        p.logNoCR(
            `Enter a valid location or press enter to use: '${_DEFAULT_HOME_POSITION_}' as default home position.`
        );
        return 'HOME_POSITION';
    }

    p.info(
        `Home position has been set to: latitude: ${vals[0]}, longitude: ${
            vals[1]
        }, altitude: ${vals[2]}.`
    );
...
const useGMapSequence = function(entryVal) {
    if (
        ...
        p.logNoCR(
            'Enter your Google Map API key (39 alpha numeric case-sensitive characters): '
        );
        return 'GMAP_API_KEY';
    }
    ...
    p.warn('/!\\ WARNINNG /!\\');
    p.warn(
        "The Monitor has been configured to use Volons' global Google Map API Key,"
    );
    p.warn(
        "This API key is shared with all volons' users but have a very limited quotas."
    );
    p.warn(
        "If the map doesn't appeare correctly in the Monitor, we recommend to use a personal Google Map API Key."
    );
...
const saveGMapAPIKeySequence = function(entryVal) {
    ...
    if (entryVal === null || entryVal.length !== 39) {
        p.logNoCR(
            `The Google map API key:[${entryVal}] is not valid. Do you want to use a personal Google Map API [Y/n] ? `
        );
        return 'USE_GMAP';
    }
...
const init = function() {
    ...
    p.logNoCR(
        `${stepNumber++}: Enter your token or press enter to generate new token: `
    );
    ...
    stdin.addListener('data', function(d) {
        ...
        switch (sequence) {
            case 'INIT':
                sequence = initSequence(entryVal, dockerCompose, droneCounter);
                break;
            case 'ADD_DRONE':
                sequence = addDroneSequence(
                    entryVal,
                    dockerCompose,
                    droneCounter
                );
            ...
            case 'DRONE_NAME':
                ...
                break;
            case 'HOME_POSITION':
                sequence = homePositionSequence(entryVal);
                break;
            case 'USE_GMAP':
                sequence = useGMapSequence(entryVal);
                break;
            case 'GMAP_API_KEY':
                sequence = saveGMapAPIKeySequence(entryVal, dockerCompose);
                break;
            default:
                break;
        }
        ...
        if (sequence === 'END') {
            ...
            p.log(
                `Next: to start Volons' docker containers run \`volons start${msgGlobal}\`.\n`
            );

            fs.writeFileSync(
                `${dockerFilePath}${_DOCKER_COMPOSE_FILENAME_}`,
                dockerCompose.toYML()
            );
...
const start = function() {
    ...
    if (stderr) {
        ...
        p.warn(
            'Entering docker-compose console (Press CTRL+C to kill all process)\n'
        );

        exec.interactive(
            `cd ${dockerFilePath};docker-compose -f ${_DOCKER_COMPOSE_FILENAME_} up`,
            err => {
                if (err) {
                    p.error(
                        'Error: Cannot execute Docker-compose command.',
                        err
                    );
                    throw err;
                }
        ...
        process.on('SIGINT', function() {
            exec.interactive(
                `cd ${dockerFilePath};docker-compose kill -f ${_DOCKER_COMPOSE_FILENAME_} -s SIGINT`,
                err => {
                    if (err) {
                        p.error(
                            'Error: Cannot execute Docker-compose command.',
...
const pull = function() {
    ...
    if (stderr) {
        ...
        exec.interactive(
            `cd ${dockerFilePath};docker-compose -f ${_DOCKER_COMPOSE_FILENAME_} pull`,
            err => {
                if (err) {
                    p.error(
                        'Error: Cannot execute Docker-compose command.',
...
const stop = function() {
    ...
    if (stderr) {
        ...
        exec.interactive(
            `cd ${dockerFilePath};docker-compose -f ${_DOCKER_COMPOSE_FILENAME_} kill -s SIGINT`,
            err => {
                if (err) {
...
const rm = function(args) {
    if (args.length !== 2) {
        printHelp('rm.txt');
        p.error(
            'Error: Project_ID parameter is missing. Usage: `volons rm [Projec ID]`.'
        );
        return;
    }
    ...
    if (store.removeDockerCompose(dockerComposeId)) {
        ...
        printHelp('rm.txt');
        p.error(
            `Error: Cannot remove project ${dockerComposeId} from Volons' project list. Sorry, you have to do it manualy.`
        );
    }
};
...
const ps = function() {
    // docker-compose ps
    exec.quiet(
        `docker-compose -f ${dockerFilePath}${_DOCKER_COMPOSE_FILENAME_} ps`,
        (stdout, stderr) => {
            if (stderr) {
                printHelp('ps.txt');
            ...
            if (!allVolonsContainers.length) {
                printHelp('ps.txt');
                p.error(
                    "Error: No container is running. Execute `volons start` to run Volons' containers."
                );
                return;
            }
            ...
            allVolonsContainers.forEach(proc => {
                ...
                if (dockerInspect.stderr) {
                    p.error(
                        '`Error: Cannot execute `docker inspect`.',
                        dockerInspect.stderr
                    );
                    return;
                }
                let inspect = {};
                try {
                    ...
                    p.error(
                        'Error: Cannot JSON.parse `docker inspect` result.',
                        execption
                    );
                    return;
                }
                ...
                if (
                    ...
                    inspect.NetworkSettings.Networks['volons-cli_volons']
                ) {
                    processInfo.IPAddress =
                        inspect.NetworkSettings.Networks[
                            'volons-cli_volons'
                        ].IPAddress;
                }
...
if (args.length === 0) {
    ...
    dockerCompose = new DockerCompose(
        dockerFilePath + _DOCKER_COMPOSE_FILENAME_,
        store.getSubnetIpInc(dockerFilePath)
    );
